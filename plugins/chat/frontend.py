...
class ChatWindow(wx.Frame):
    def __init__(self, parent):
        super(ChatWindow, self).__init__(parent, title="Chat Window", size=(800, 700))
    ...
    def handle_user_input(self):
        ...
        try:
            ...
            QA_gpt_messages = [
                {"role": "system", "content": "You are a helpful assistant from Smarton Company, which is designed to help pcb engineers to learn Schematic Editor and PCB Editor for Kicad."},
                # {"role": "system", "content": "You have two source of response, 'Google Search' and 'Kicad Documentation'. In most cases, your source of response will be 'Google Search'"},
                {"role": "system", "content": "You should answer the question mainly based on the Kicad Documentation content, if the content does not mention the user's question, you can take your own knowledge which is from Google to the consideration necessarily."},
            ]

            # compute the number of content for each page
            # num_of_content = 3600 // len(html_text_content_list)
            num_of_content = 1000
            QA_gpt_messages += [{"role": "system", "content": f"Kicad Documentation content:\n{html_text_content[:num_of_content]}"} for html_text_content in html_text_content_list]
            ...
            self.button_Disable()
            qa_first_input = f"give your response to the question '{main_task_description}'"
            # qa_first_input = qa_first_input + f"and give the source (either 'Google Search' and 'Kicad Documentation') of your response using parentheses only in the last line, never give the url of the source."
            qa_gpt_response_main = QA_model.ask_gpt(qa_first_input)
            ...
            while i > 0:
                ...
                if i > 2 and QA_user_input == 'add':
                    ...
                    wx.CallAfter(self.chat_display.AppendText, f"==========  Smarton AI  ==========\n")
                    Data_Increment_GPTModel_messages = data_increment_gpt_preinput.data_increment_gpt_preinput()
                    DI_GPT = Data_increment_GPTModel(model='gpt-3.5-turbo-0613', messages=Data_Increment_GPTModel_messages, language=self.language, QA_model=QA_model, Task_plugins=self.Task_plugins)
                    QA_model = DI_GPT.add_new_knowledge(recent_chat, wx.CallAfter, self.chat_display.AppendText, self.DataIncrement_act, self.DataIncrement_dis)
                else:
                    self.button_Disable()
    ...
    def DataIncrement_act(self):
        self.send_button.Enable()

    def DataIncrement_dis(self):
        self.send_button.Disable()
